...
#         self.augment_data('test')
    ...
    def predict(self):
        ...
        #         self.clf = pickle.load(clf_file)
        # else:
        #     print("Classifier file not provided.")
        #
    ...
    def test_best_model(self):
        ...
        pred_y = model.predict(cnn_data(self.test_X))

        threshold = 0.5
        binary_predictions = (pred_y > threshold).astype(int)
        print(binary_predictions)
        print('CNN accuracy:', accuracy_score(self.test_y, binary_predictions, normalize=False))
        print('CNN confusion matrix\n', confusion_matrix(self.test_y, binary_predictions, normalize=None))
        self.test_model_significance(binary_predictions)

        plot_confusion_matrix(confusion_matrix(self.test_y, binary_predictions), target_names=['sober', 'drunk'])
        import time; time.sleep(1)
        self.demo_model(binary_predictions)
